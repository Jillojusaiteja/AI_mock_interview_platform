...
// Allow the frontend during development to run on different localhost ports
...
app.use(
	cors({
		origin: (origin, callback) => {
			...
			// In development, accept any localhost origin (ports may change)
			if ((process.env.NODE_ENV || 'development') === 'development' && origin.startsWith('http://localhost')) {
				return callback(null, true);
			}
			// Otherwise only allow the configured client URL
